...
$params = @{
	...
	skip = 0
	top = 50
	filter = "((PolicyBaseTypeName eq 'Microsoft.Management.Services.Api.DeviceConfiguration') or (PolicyBaseTypeName eq 'DeviceManagementConfigurationPolicy') or (PolicyBaseTypeName eq 'DeviceConfigurationAdmxPolicy')) 
and (PolicyId eq 'fa534cd6-4d15-4d41-bf9d-43f2bf742116')"
	orderBy = @(
	)
}
