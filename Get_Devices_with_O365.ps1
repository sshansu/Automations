...
## Device URL
$url1 = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$filter=(Notes eq 'bc3e5c73-e224-4e63-9b2b-0c36784b7e80') and (deviceType eq 'desktop') `
or (deviceType eq 'windowsRT')" 

$result = Invoke-RestMethod -Uri $url1  -Method Get -Headers $global:authToken
$devices = $result.value | select deviceName, id
$table2 = @()
foreach($id in $devices.id)
    {
        $url2 = "https://graph.microsoft.com/beta/deviceManagement/manageddevices('$id')?`$expand=detectedApps"
        $result = Invoke-RestMethod -Method Get -Uri $url2 -Headers $global:authToken
        $O365Version = $result.detectedApps | where{$_.displayName -eq 'Microsoft 365 Apps for enterprise - en-us'} | select displayName, version
        
        $table2 += New-Object -TypeName PSCustomObject -Property @{
        DeviceName = $devices | where{$_.id -eq $id} | select deviceName -ExpandProperty deviceName
        App = $O365Version.displayName
        Version = $O365Version.version
        }
    }

$table2 | Export-Csv C:\Temp\O365.csv -Append -NoTypeInformation