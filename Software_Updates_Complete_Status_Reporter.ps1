...
In no event shall Microsoft, its authors, or anyone else involved in the creation, production, or delivery of the scripts be liable for any damages whatsoever (including, #without limitation, 
damages for loss of business profits, business interruption, loss of business information, or other pecuniary loss) arising out of the use of or inability to use the sample scripts or 
documentation, even if Microsoft has been advised of the possibility of such damages

This script runs locally on a Windows machine and captures information that helps us guage the current status of Software updates from each pc.
This creates custom log analytics tables as outlined from lines 46 to 54.
#>
...
Function Write-Log
{
    ...
    PARAM(
         [String]$Message,
         [String]$Path = "C:\SystemUpdateReport\$($env:computername)-UpdateStatusUpdater.Log",
         [int]$severity,
         [string]$component
         )
...
$WorkspaceID = "" # WorkspaceID of the Log Analytics workspace
$PrimaryKey = "" # Primary Key of the Log Analytics workspace
$ParentDirectoryName = "" # eg Company or domain name
...
{
    write-Log -Message "Another execution is currently running" -severity 1 -component "UpdateReporter" 
    Exit 0
}
else 
{
...
If ($LatestDeltaInventoryDate -and $LatestFullInventoryDate)
{
    ...
    Else  
    {
        write-Log -Message "No scheduled inventory required at this run" -severity 1 -component "UpdateReporter" 
...
{
    write-Log -Message "No Intune device Id could be found for this device" -severity 1 -component "UpdateReporter" 
...
foreach ($UniqueUpdateName in $UniqueUpdateNames)
{
    ...
    $FinalEventArray += $UpdateEventArrayList.Where({$_.UpdateName -eq $UniqueUpdateName}) | where{$_.UpdateName -eq "Windows 11"} | Sort-Object -Property TimeCreated -Descending | Select -First 1 
}
...
If ($InventoryType -eq "Delta")
{
    ...
    If ($LatestFullInventoryFile)
    {
        ...
        else 
        {
            ...
            {
                $Stopwatch.Stop()
                write-Log -Message "Delta contains no new data"  -severity 1 -component "UpdateReporter" 
...
If ($UnsuccessfullySent -ge 1)
{
    ...
    Set-ItemProperty -Path $FullRegPath -Name ExecutionStatus -Value "" -Force
    write-Log -Message "$($UnsuccessfulItems)"  -severity 1 -component "UpdateReporter" 
    Exit 1
}
else 
{
    ...
    write-Log -Message "$($InventoryType) inventory sent at $(Get-Date -Format ""s""). Category count: $($SuccessfullySent)"  -severity 1 -component "UpdateReporter" 
    Set-ItemProperty -Path $FullRegPath -Name ExecutionStatus -Value "" -Force
}
#endregion
