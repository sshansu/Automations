...
$JSONs = Get-ChildItem "C:\Users\sshansu\OneDrive - Microsoft\CX\_Lab" -Filter *.json
foreach($FileName in $JSONs)
    {
        If (Test-Path -Path $FileName.FullName -Type Leaf) {
	        $ImportPath = $FileName.FullName
        } Else {
