...
foreach($policy in $policies)
    {
        write-host "Deleting $($policy.displayName)"

        #delete configuration profile
        $url1 = "https://graph.microsoft.com/beta/deviceManagement/deviceConfigurations/$($policy.id)"
        Invoke-RestMethod -Uri $Url1 -Method Delete -Headers $global:authToken
    }