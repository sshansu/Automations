...
$global:ClientID = ""
$global:ClientSecret = ""
$exportfile = "C:\Temp\Merged-$(Get-Date -Format "MM-dd-yyyy_hh-mm-ss").csv"
...
param($DeviceName)
    ...
    $results = $Output.value | select deviceName, isEncrypted -Unique
    if($results)
        {
        ...
        {
            write-host "$DeviceName does not exist" -ForegroundColor Red
        }
}
$hash = @()
$Group = "All Devices"

$TotalDevices = Get-GroupMembership -GroupName $Group
$Devices = $($TotalDevices.displayName | select -Unique)
Write-host "Total Devices: $($Devices.count)" -ForegroundColor Cyan
Write-host "Getting results..." -ForegroundColor Yellow
foreach($Device in $Devices)
    {
        write-host "Working on $Device" -ForegroundColor Cyan
        $Results = Get-EncryptionState -DeviceName $Device 
        $hash += New-Object -TypeName PSCustomObject -Property @{
                DeviceName = $Results.deviceName
                Encrypted = $Results.isEncrypted
        }
    }
 $hash | select DeviceName, Encrypted | where{$_.Encrypted -ne $Null} | Export-Csv -Path $exportfile -Append -NoTypeInformation 
